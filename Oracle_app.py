...
def genera_output(parte, item, identificativo, classe, catalog, erp_l2,
    ...
    # --- Ripristino Catalog e Disegno ---
    catalog_input = st.text_input("Catalog", value=catalog, key=f"catalog_{parte}")
    dwg_input     = st.text_input("Disegno", key=f"dwg_{parte}")
...
if selected_part == "Baseplate, Pump":
    ...
    dwg_fp      = st.text_input("Disegno", key="dwg_flange")
...
# … ripeti per Gate, Gasket, Bearing… usando lo stesso schema: *
#   - input dwg = st.text_input("Disegno", …)
#   - nell’output_data reinserire "Catalog" e "Disegno" 
#   - per le parti che usano genera_output, il campo Catalog è già gestito
