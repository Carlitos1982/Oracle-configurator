...
def render_dataload_panel(item_code_key: str, create_btn_key: str, update_btn_key: str, state_key: str='output_data'):
    st.subheader('🧾 DataLoad')
    mode = st.radio('Operation type:', ['Create new item', 'Update existing item'], key=f'{item_code_key}_mode')
    ...
    if mode == 'Create new item':
        if st.button('Generate DataLoad string', key=create_btn_key):
            if not item_code:
                ...
                fields = ['\\%FN', item_code, '\\%TC', get_val('Template'), 'TAB', '\\%D', '\\%O', 'TAB', get_val('Description'), *['TAB'] * 6, get_val('Identificativo'), 'TAB', get_val('Classe ricambi'), 'TAB', '\\%O', '\\^S', '\\%TA', 'TAB', f"{get_val('ERP_L1')}.{get_val('ERP_L2')}", 'TAB', 'FASCIA ITE', 'TAB', item_code[:1], 'TAB', '\\^S', '\\^{F4}', '\\%TG', get_val('Catalog'), *['TAB'] * 4, get_val('Disegno'), 'TAB', '\\^S', '\\^{F4}', '\\%TR', 'MATER+DESCR_FPD', *['TAB'] * 2, get_val('FPD material code'), 'TAB', get_val('Material'), '\\^S', '\\^S', '\\^{F4}', '\\%VA', 'TAB', 'Quality', *['TAB'] * 4, *quality_tokens, '\\^S', '\\^{F4}', '\\^S']
                st.success('✅ DataLoad string successfully generated. Download the CSV file below.')
                buf = io.StringIO()
                writer = csv.writer(buf, quoting=csv.QUOTE_MINIMAL)
                for tok in fields:
                    writer.writerow([tok])
                st.download_button('💾 Download CSV for Import', data=buf.getvalue(), file_name=f'dataload_{item_code}.csv', mime='text/csv')
    elif st.button('Generate Update string', key=update_btn_key):
        if not item_code:
            st.error('❌ Please enter the item code first.')
        else:
            fields = ['\\%VF', item_code, '\\{NUMPAD ENTER}', 'TAB', get_val('Description', '*?'), *['TAB'] * 6, get_val('Identificativo'), 'TAB', get_val('Classe ricambi'), 'TAB', '\\%O', '\\^S', '\\%TA', '\\%VF', 'FASCIA ITE', '\\{NUMPAD ENTER}', 'TAB', item_code[:1], '\\^S', '\\%VF', 'TIPO ARTICOLO', '\\{NUMPAD ENTER}', 'TAB', f"{get_val('ERP_L1')}.{get_val('ERP_L2')}", '\\^S', '\\^{F4}', '\\%TG', get_val('Catalog'), *['TAB'] * 3, get_val('Disegno'), 'TAB', '\\^S', '\\^{F4}', '\\^S', '\\%VA', 'TAB', 'Quality', *['TAB'] * 4, *quality_tokens, '\\^S', '\\^{F4}', '\\^S']
            st.success('✅ Update string successfully generated. Download the CSV file below.')
            buf = io.StringIO()
            writer = csv.writer(buf, quoting=csv.QUOTE_MINIMAL)
            for tok in fields:
                writer.writerow([tok])
            st.download_button('💾 Download CSV for Update', data=buf.getvalue(), file_name=f'update_{item_code}.csv', mime='text/csv')
...
with col_header:
    st.markdown('<h1 style="white-space: nowrap; margin: 0; font-size: 2rem;">Oracle Item Setup - Web App</h1>', unsafe_allow_html=True)
with col_logo:
...
def get_fpd_code(mat_type, mat_prefix, mat_name):
    row = materials_df[(materials_df['Material Type'] == mat_type) & (materials_df['Prefix'] == mat_prefix) & (materials_df['Name'] == mat_name)]
    if not row.empty and 'FPD Code' in row.columns:
        return row.iloc[0]['FPD Code']
    return 'NOT AVAILABLE'
bolt_sizes = ['#10-24UNC', '5/16"-18UNC', '3/8"-16UNC', '1/2"-13UNC', '3/4"-16UNF', '7/8"-9UNC', '7/8"-14UNF', '1"-12UNF', '1-1/8"-12UNF', '1-1/2"-12UNC', '2"-4.5UNC', '2-1/2"-4UNC', '3"-6UNC', '4"-8UNC', 'M6x1', 'M8x1.25', 'M10x1.5', 'M12x1.75', 'M16x2', 'M20x2.5', 'M24x3', 'M30x3.5', 'M36x4', 'M42x4.5', 'M48x5', 'M56x5.5', 'M64x6', 'M72x6', 'M80x6', 'M90x6', 'M100x6']
bolt_lengths = ['1/8"in', '1/4"in', '3/8"in', '5/16"in', '1/2"in', '3/4"in', '1"in', '1-1/8"in', '1-1/4"in', '1-3/8"in', '1-1/2"in', '2"in', '2-1/8"in', '2-1/4"in', '2-3/8"in', '2-1/2"in', '2-3/4"in', '3"in', '3-1/8"in', '3-1/4"in', '3-3/8"in', '3-1/2"in', '4"in', '4-1/8"in', '4-1/4"in', '4-3/8"in', '4-1/2"in', '50mm', '55mm', '60mm', '65mm', '70mm', '75mm', '80mm', '85mm', '90mm', '95mm', '100mm', '105mm', '110mm', '115mm', '120mm', '125mm', '130mm', '135mm', '140mm', '145mm', '150mm', '155mm', '160mm', '165mm', '170mm', '175mm', '180mm', '185mm', '190mm', '195mm']
skf_models = ['6000', '6001', '6002', '6003', '6004', '6005', '6006', '6007', '6008', '6009', '6010', '6200', '6201', '6202', '6203', '6204', '6205', '6206', '6207', '6208', '6209', '6210', '6211', '6212', '6300', '6301', '6302', '6303', '6304', '6305', '6306', '6307', '6308', '6309', '6310', '6311', '6312', '3200', '3201', '3202', '3203', '3204', '3205', '3206', '3207', '3208', '3209', '3210', '7200', '7201', '7202', '7203', '7204', '7205', '7206', '7207', '7208', '7209', '7210', '7211', '7212', '7300', '7301', '7302', '7303', '7304', '7305', '7306', '7307', '7308', '7309', '7310', '7311', '7312', '1200', '1201', '1202', '1203', '1204', '1205', '1206', '1207', '1208', '1209', '1210', '2200', '2201', '2202', '2203', '2204', '2205', '2206', '2207', '2208', '2209', '2210', 'NU202', 'NU203', 'NU204', 'NU205', 'NU206', 'NU207', 'NU208', 'NU209', 'NU210', 'NUP202', 'NUP203', 'NUP204', 'NUP205', 'NUP206', 'NUP207', 'NUP208', 'NUP209', 'NUP210', 'NJ202', 'NJ203', 'NJ204', 'NJ205', 'NJ206', 'NJ207', 'NJ208', 'NJ209', 'NJ210', '22205', '22206', '22207', '22208', '22209', '22210', '22211', '22212', '22308', '22309', '22310', '22311', '22312', '23022', '23024', '23026', '23120', '23122', '23124', '30205', '30206', '30207', '30208', '30209', '30210', '30211', '30212', '30305', '30306', '30307', '30308', '30309', '30310', '32005', '32006', '32007', '32008', '32009', '32010', '32011', '32012']
skf_seals = ['', '2RS1', '2RSH', '2RSL', 'RS1', 'RS', 'Z', 'ZZ', '2Z']
skf_design = ['', 'BE (40° AC, paired)', 'B (40° AC)', 'AC (25° AC)', 'A (30° AC)', 'E (high capacity)', 'EC (high capacity)']
skf_pairing = ['', 'CB (light preload)', 'CC (medium preload)', 'CD (heavy preload)', 'GA (paired)', 'GB (paired)', 'GC (paired)']
skf_cages = ['', 'TN9 (polyamide)', 'J (pressed steel)', 'M (machined brass)', 'MA (brass)', 'CA (brass)', 'CC (polyamide)']
...
base_series_desc = {'60': 'Deep groove ball bearing – light series', '62': 'Deep groove ball bearing – medium series', '63': 'Deep groove ball bearing – heavy series', '72': 'Angular contact ball bearing – 15°', '73': 'Angular contact ball bearing – 40°', '32': 'Double-row angular contact ball bearing', '12': 'Self-aligning ball bearing – light series', '22': 'Self-aligning ball bearing – medium series', 'NU': 'Cylindrical roller bearing (NU)', 'NUP': 'Cylindrical roller bearing (NUP)', 'NJ': 'Cylindrical roller bearing (NJ)', '222': 'Spherical roller bearing – 222 series', '223': 'Spherical roller bearing – 223 series', '230': 'Spherical roller bearing – 230 series', '231': 'Spherical roller bearing – 231 series', '302': 'Tapered roller bearing – 302 series', '303': 'Tapered roller bearing – 303 series', '320': 'Tapered roller bearing – 320 series'}
design_desc = {'BE': '40° AC, paired', 'B': '40° AC', 'AC': '25° AC', 'A': '30° AC', 'E': 'high capacity', 'EC': 'high capacity'}
pairing_desc = {'CB': 'light preload', 'CC': 'medium preload', 'CD': 'heavy preload', 'GA': 'paired', 'GB': 'paired', 'GC': 'paired'}
cage_desc = {'TN9': 'polyamide cage', 'J': 'pressed steel cage', 'M': 'machined brass cage', 'MA': 'brass cage', 'CA': 'brass cage', 'CC': 'polyamide cage'}
clearance_desc = {'C2': 'reduced clearance', 'CN': 'normal clearance', 'C3': 'increased clearance', 'C4': 'large clearance', 'C5': 'very large clearance'}
...
vibration_desc = {'V1': 'V1 vib.', 'V2': 'V2 vib.', 'V3': 'V3 vib.', 'V4': 'V4 vib.', 'VA201': 'VA201 vib.', 'VA208': 'VA208 vib.', 'VA228': 'VA228 vib.'}
dowel_diameters_mm_raw = ['Ø1', 'Ø1.5', 'Ø2', 'Ø2.5', 'Ø3', 'Ø4', 'Ø5', 'Ø6', 'Ø8', 'Ø10', 'Ø12', 'Ø14', 'Ø16', 'Ø18', 'Ø20', 'Ø22', 'Ø25', 'Ø30']
dowel_lengths_mm = ['4mm', '5mm', '6mm', '8mm', '10mm', '12mm', '16mm', '20mm', '25mm', '30mm', '35mm', '40mm', '45mm', '50mm', '60mm', '70mm', '80mm', '90mm', '100mm']
dowel_diameters_in = ['1/16"', '3/32"', '1/8"', '5/32"', '3/16"', '7/32"', '1/4"', '9/32"', '5/16"', '3/8"', '7/16"', '1/2"', '9/16"', '5/8"', '3/4"', '7/8"', '1"']
dowel_lengths_in = ['1/4"', '3/8"', '1/2"', '5/8"', '3/4"', '1"', '1-1/4"', '1-1/2"', '1-3/4"', '2"', '2-1/4"', '2-1/2"', '3"', '3-1/2"', '4"']
winding_materials = ['SS316L', 'SS304', 'MONEL', 'INCONEL', 'DUPLEX', 'HASTELLOY C276']
filler_materials = ['GRAPHITE', 'PTFE', 'MICA', 'CERAMIC', 'GLASS', 'SS304']
color_codes = {'SS316L': 'Green', 'SS304': 'Red', 'MONEL': 'Blue', 'INCONEL': 'Yellow', 'DUPLEX': 'Purple', 'HASTELLOY C276': 'Orange', 'GRAPHITE': 'Black', 'PTFE': 'White', 'MICA': 'Gray', 'CERAMIC': 'Light Gray', 'GLASS': 'Clear'}
categories = {'Machined Parts': ['Baseplate, Pump', 'Casing, Pump', 'Casing Cover, Pump', 'Impeller, Pump', 'Balance Bushing, Pump', 'Balance Drum, Pump', 'Balance Disc, Pump', 'Shaft, Pump', 'Ring, Wear'], 'Piping': ['Flange, Pipe', 'Gate, Valve', 'Gasket, Flat', 'Gasket, Ring Type Joint'], 'Fasteners': ['Bolt, Eye', 'Bolt, Hexagonal', 'Nut, Hex', 'Stud, Threaded', 'Screw, Cap', 'Screw, Grub', 'Pin, Dowel'], 'Commercial Parts': ['Bearing, Hydrostatic/Hydrodynamic', 'Bearing, Rolling', 'Gusset, Other', 'Gasket, Spiral Wound'], 'Casting': ['Casing cover casting', 'Casing casting', 'Bearing housing casting', 'Impeller casting', 'Impeller nut casting', 'Shaft casting', 'Throttling bush casting', 'Pump bowl casting', 'Bearing bracket casting', 'Discharge elbow casting', 'Bearing cover casting', 'Diffuser casting', 'Inducer casting', 'Wear plate casting', 'Shaft wear sleeve casting']}
col1, col2 = st.columns([1, 1], gap='small')
with col1:
    selected_category = st.selectbox('Category:', [''] + list(categories.keys()), index=0, key='selectbox_1')
with col2:
    if selected_category:
        part_list = categories[selected_category]
    else:
        part_list = []
    selected_part = st.selectbox('Part:', [''] + part_list, key='selected_part')
...
if selected_part == 'Casing, Pump':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='casing_gen'):
            ...
            quality_lines = ['SQ 58 - Controllo Visivo e Dimensionale delle Lavorazioni Meccaniche', 'CORP-ENG-0115 - General Surface Quality Requirements G1-1']
            if hf_service:
            ...
            st.session_state['output_data'] = {'Item': '40201…', 'Description': descr, 'Identificativo': '1100-CASING', 'Classe ricambi': '3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'CORPO', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_MAKE', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '17_CASING', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
    ...
    with col3:
        render_dataload_panel(item_code_key='beye_item_code', create_btn_key='gen_dl_beye', update_btn_key='gen_upd_beye')
    ...
    with col1:
        ...
        if st.button('Generate Output', key='ccov_gen'):
            ...
            if material_note:
                descr_parts.append(material_note)
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40205…', 'Description': descr, 'Identificativo': '1221-CASING COVER', 'Classe ricambi': '3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'CORPO', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': template, 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '17_CASING', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
        ...
        if st.button('Generate Output', key='imp_gen'):
            ...
            for val in [model, size, feature_1, note, materiale, material_note]:
                if val:
                    descr_parts.append(val)
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40229…', 'Description': descr, 'Identificativo': '2200-IMPELLER', 'Classe ricambi': '2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ARTVARI', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_MAKE', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '20_IMPELLER_DIFFUSER', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
        ...
        if st.button('Generate Output', key='bbush_gen'):
            ...
            for val in [model, size, feature_1, note, materiale, material_note]:
                if val:
                    descr_parts.append(val)
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40226…', 'Description': descr, 'Identificativo': '6231-BALANCE DRUM BUSH', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ALBERO', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_1', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '16_BUSHING', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
        ...
        if st.button('Generate Output', key='bdrum_gen'):
            ...
            for val in [model, size, feature_1, note, materiale, material_note]:
                if val:
                    descr_parts.append(val)
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40226…', 'Description': descr, 'Identificativo': '6230-BALANCE DRUM', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ARTVARI', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_1', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '16_BUSHING', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
        ...
        if st.button('Generate Output', key='bdisc_gen'):
            ...
            for val in [model, size, feature_1, note, materiale, material_note]:
                if val:
                    descr_parts.append(val)
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40226…', 'Description': descr, 'Identificativo': '6210-BALANCE DISC', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ARTVARI', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_1', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '30_DISK', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Gate, Valve':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='gate_gen'):
            ...
            descr += f' {tag_string}'
            descr = '*' + descr
            st.session_state['output_data'] = {'Item': '50186…', 'Description': descr, 'Identificativo': 'VALVOLA (GLOBO,SARAC,SFERA,NEEDLE,MANIF,CONTR)', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_2', 'ERP_L1': '72_VALVE', 'ERP_L2': '18_GATE_VALVE', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Gasket, Spiral Wound':
    ...
    with col1:
        st.subheader('✏️ Input')
        winding_options = {'304 stainless steel': ('Yellow', 'RAL1021'), '316L stainless steel': ('Green', 'RAL6005'), '317L stainless steel': ('Maroon', 'RAL3003'), '321 stainless steel': ('Turquoise', 'RAL5018'), '347 stainless steel': ('Blue', 'RAL5017'), 'MONEL': ('Orange', 'RAL2003'), 'Nickel': ('Red', 'RAL3024'), 'Titanium': ('Purple', 'RAL4003'), 'Alloy 20': ('Black', 'RAL9005'), 'INCONEL 600': ('Gold', 'RAL1004'), 'HASTELLOY B': ('Brown', 'RAL8003'), 'HASTELLOY C': ('Beige', 'RAL1011'), 'INCOLOY 800': ('White', 'RAL9010'), 'DUPLEX': ('Yellow+Blue', 'RAL1021+5017'), 'SUPERDUPLEX': ('Red+Black', 'RAL3020+9005'), 'ALLOY 825': ('Orange+Green', 'RAL2003+6005'), 'UNS S31254': ('Orange+Blue', 'RAL2003+5017'), 'ZYRCONIUM 702': ('Gold+Green', 'RAL1004+6005'), 'INCONEL X750HT': ('Gold+Black', 'RAL1004+9005')}
        filler_options = {'Graphite': ('Gray', 'RAL7011'), 'PTFE': ('White', 'RAL9010'), 'Ceramic': ('Light Green', 'RAL6021'), 'Verdicarb (Mica Graphite)': ('Pink', 'RAL3015')}
        rating_mapping = {'STANDARD PRESSURE - m=3; y=10000psi (1 stripe)': ('STANDARD PRESSURE', 'm=3; y=10000psi', '1 stripe'), 'HIGH PRESSURE - m=3; y=17500psi (2 stripes)': ('HIGH PRESSURE', 'm=3; y=17500psi', '2 stripes'), 'ULTRA HIGH PRESSURE - m=3; y=23500psi (3 stripes)': ('ULTRA HIGH PRESSURE', 'm=3; y=23500psi', '3 stripes')}
        ...
        if st.button('Generate Output', key='gsw_gen'):
            ...
            descr_gsw = f'*GASKET, SPIRAL WOUND - WINDING: {winding_gsw}, FILLER: {filler_gsw}, OD: {out_dia_gsw} MM, ID: {in_dia_gsw} MM, THK: {thickness_gsw} MM, RATING: {pressure_label} – {rating_descr}, COLOR CODE: {color1} {ral1} / {color2} {ral2} ({stripe})'
            if note_gsw:
                descr_gsw += f', {note_gsw}'
            qual_tags = ['[SQ174]']
            quality_lines = ['SQ 174 - Casing/Cover pump spiral wound gaskets: Specification for Mechanical properties, applicable materials and dimensions']
            if hf_service_gsw:
                qual_tags.append('<SQ113>')
                quality_lines.append('SQ 113 - Material Requirements for Pumps in Hydrofluoric Acid Service (HF)')
            descr_gsw += ' ' + ' '.join(qual_tags)
            quality_field = '\n'.join(quality_lines)
            st.session_state['output_data'] = {'Item': '50415…', 'Description': descr_gsw, 'Identificativo': '4510-JOINT', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg_gsw, 'Material': 'BUY OUT NOT AVAILABLE', 'FPD material code': 'BO-NA', 'Template': 'FPD_BUY_1', 'ERP_L1': '55_GASKETS_OR_SEAL', 'ERP_L2': '16_SPIRAL_WOUND', 'To supplier': '', 'Quality': quality_field}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
...
if selected_part == 'Bearing, Hydrostatic/Hydrodynamic':
    ...
    with col1:
        ...
        pref_df_bear = materials_df[(materials_df['Material Type'] == mtype_bear) & materials_df['Prefix'].notna()]
        ...
        if mtype_bear == 'MISCELLANEOUS':
            ...
            names_bear = materials_df[(materials_df['Material Type'] == mtype_bear) & (materials_df['Prefix'] == mprefix_bear)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='bear_gen'):
            materiale_bear = mname_bear if mtype_bear == 'MISCELLANEOUS' else f'{mtype_bear} {mprefix_bear} {mname_bear}'.strip()
            match_bear = materials_df[(materials_df['Material Type'] == mtype_bear) & (materials_df['Prefix'] == mprefix_bear) & (materials_df['Name'] == mname_bear)]
            codice_fpd_bear = match_bear['FPD Code'].values[0] if not match_bear.empty else ''
            dim_bear = ' - '.join([f'OD {od_bear}' if od_bear else '', f'ID {id_bear}' if id_bear else '', f'W {width_bear}' if width_bear else '']).strip(' -')
            descr_parts_bear = ['BEARING, HYDROSTATIC/HYDRODYNAMIC', dim_bear, note_bear, materiale_bear, material_note_bear]
            descr_bear = '*' + ' - '.join([p for p in descr_parts_bear if p])
            st.session_state['output_data'] = {'Item': '50122…', 'Description': descr_bear, 'Identificativo': '3010-ANTI-FRICTION BEARING', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ALBERO', 'Disegno': dwg_bear, 'Material': materiale_bear, 'FPD material code': codice_fpd_bear, 'Template': 'FPD_BUY_1', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '29_OTHER', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Bearing, Rolling':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='br_gen'):
            ...
            parts_no_space = [model_final, code_seal, code_design, code_pairing, code_cage, code_clear, code_tol, code_heat, code_grease, code_vib, extra_suffix.strip()]
            skf_full_code = ''.join([p for p in parts_no_space if p]).upper()
            bearing_type_txt = bearing_type_from_code(model_final)
            desc_bits = [design_desc.get(code_design, ''), pairing_desc.get(code_pairing, ''), cage_desc.get(code_cage, ''), clearance_desc.get(code_clear, ''), tolerance_desc.get(code_tol, ''), heat_desc.get(code_heat, ''), grease_desc.get(code_grease, ''), vibration_desc.get(code_vib, '')]
            ...
            dim_roll = ' - '.join([f'OD {od_roll}' if od_roll else '', f'ID {id_roll}' if id_roll else '', f'W {width_roll}' if width_roll else '']).strip(' -')
            descr_parts_roll = ['BEARING, ROLLING', skf_full_code + human_suffix, dim_roll, note_roll]
            descr_roll = '*' + ' - '.join([p for p in descr_parts_roll if p])
            st.session_state['output_data'] = {'Item': '50122…', 'Description': descr_roll, 'Identificativo': '3010-ANTI-FRICTION BEARING', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': 'COMMERCIAL BEARING', 'FPD material code': 'NA', 'Template': 'FPD_BUY_2', 'ERP_L1': '31_COMMERCIAL_BEARING', 'ERP_L2': '18_OTHER', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Bolt, Eye':
    ...
    with col1:
        ...
        names_beye = materials_df[materials_df['Material Type'] == mtype_beye]['Name'].dropna().tolist() if mtype_beye == 'MISCELLANEOUS' else materials_df[(materials_df['Material Type'] == mtype_beye) & (materials_df['Prefix'] == mprefix_beye)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='beye_gen'):
            ...
            match = materials_df[(materials_df['Material Type'] == mtype_beye) & (materials_df['Prefix'] == mprefix_beye) & (materials_df['Name'] == mname_beye)]
            codice_fpd = match['FPD Code'].values[0] if not match.empty else ''
            ...
            st.session_state['output_data'] = {'Item': '55150…', 'Description': descr, 'Identificativo': '6583-EYE BOLT', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Bolt, Hexagonal':
    ...
    with col1:
        ...
        if mtype_bh == 'MISCELLANEOUS':
            ...
            names_bh = materials_df[(materials_df['Material Type'] == mtype_bh) & (materials_df['Prefix'] == mprefix_bh)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='bh_gen'):
            materiale_bh = mname_bh if mtype_bh == 'MISCELLANEOUS' else f'{mtype_bh} {mprefix_bh} {mname_bh}'.strip()
            match_bh = materials_df[(materials_df['Material Type'] == mtype_bh) & (materials_df['Prefix'] == mprefix_bh) & (materials_df['Name'] == mname_bh)]
            codice_fpd_bh = match_bh['FPD Code'].values[0] if not match_bh.empty else ''
            descr_parts_bh = ['HEXAGONAL BOLT', size_bh, length_bh, 'FULL THREADED' if full_thread_bh == 'Yes' else '', note_bh, materiale_bh, 'ZINC PLATED' if zinc_plated_bh == 'Yes' else '', material_note_bh]
            descr_bh = '*' + ' - '.join([p for p in descr_parts_bh if p])
            st.session_state['output_data'] = {'Item': '56230…', 'Description': descr_bh, 'Identificativo': '6577-HEXAGON HEAD BOLT', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': materiale_bh, 'FPD material code': codice_fpd_bh, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Gasket, Ring Type Joint':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='rtj_gen'):
            descr_rtj = f'GASKET, RTJ - STYLE: {style_rtj}, SIZE: {size_rtj}, MATERIAL: {material_rtj}'
            if note_rtj:
            ...
            if hf_service_rtj:
                ...
                quality = ''
            descr_rtj = '*' + descr_rtj
            st.session_state['output_data'] = {'Item': '50158…', 'Description': descr_rtj, 'Identificativo': '4595-JOINT RING', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg_rtj, 'Material': material_rtj, 'FPD material code': 'NOT AVAILABLE', 'Template': 'FPD_BUY_1', 'ERP_L1': '55_GASKETS_OR_SEAL', 'ERP_L2': '20_OTHER', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
    ...
    with col1:
        ...
        if mtype_gusset == 'MISCELLANEOUS':
            ...
            names_gusset = materials_df[(materials_df['Material Type'] == mtype_gusset) & (materials_df['Prefix'] == mprefix_gusset)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='gen_gusset'):
            if mtype_gusset != 'MISCELLANEOUS':
                ...
                match_gusset = materials_df[(materials_df['Material Type'] == mtype_gusset) & (materials_df['Prefix'] == mprefix_gusset) & (materials_df['Name'] == mname_gusset)]
            else:
                materiale_gusset = mname_gusset
                match_gusset = materials_df[(materials_df['Material Type'] == mtype_gusset) & (materials_df['Name'] == mname_gusset)]
            codice_fpd_gusset = match_gusset['FPD Code'].values[0] if not match_gusset.empty else ''
            descr_gusset = f'GUSSET, OTHER - WIDTH: {int(width_gusset)}{uom_gusset}, THK: {int(thickness_gusset)}{uom_gusset}'
            if note1_gusset:
            ...
            st.session_state['output_data'] = {'Item': '565G…', 'Description': descr_gusset, 'Identificativo': 'GUSSETING', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': materiale_gusset, 'FPD material code': codice_fpd_gusset, 'Template': 'FPD_BUY_1', 'ERP_L1': '21_FABRICATION_OR_BASEPLATES', 'ERP_L2': '29_OTHER', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for campo, valore in st.session_state['output_data'].items():
...
if selected_part == 'Stud, Threaded':
    ...
    with col1:
        ...
        pref_df_stud = materials_df[(materials_df['Material Type'] == mtype_stud) & materials_df['Prefix'].notna()]
        ...
        if mtype_stud == 'MISCELLANEOUS':
            ...
            names_stud = materials_df[(materials_df['Material Type'] == mtype_stud) & (materials_df['Prefix'] == mprefix_stud)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='stud_gen'):
            materiale_stud = mname_stud if mtype_stud == 'MISCELLANEOUS' else f'{mtype_stud} {mprefix_stud} {mname_stud}'.strip()
            match_stud = materials_df[(materials_df['Material Type'] == mtype_stud) & (materials_df['Prefix'] == mprefix_stud) & (materials_df['Name'] == mname_stud)]
            ...
            st.session_state['output_data'] = {'Item': '56146…', 'Description': descr_stud, 'Identificativo': '6572-STUD', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg_stud, 'Material': materiale_stud, 'FPD material code': codice_fpd_stud, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Nut, Hex':
    ...
    with col1:
        ...
        if mtype_nut == 'MISCELLANEOUS':
            ...
            names_nut = materials_df[(materials_df['Material Type'] == mtype_nut) & (materials_df['Prefix'] == mprefix_nut)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='nut_gen'):
            materiale_nut = mname_nut if mtype_nut == 'MISCELLANEOUS' else f'{mtype_nut} {mprefix_nut} {mname_nut}'.strip()
            match_nut = materials_df[(materials_df['Material Type'] == mtype_nut) & (materials_df['Prefix'] == mprefix_nut) & (materials_df['Name'] == mname_nut)]
            ...
            st.session_state['output_data'] = {'Item': '56030…', 'Description': descr_nut, 'Identificativo': '6581-HEXAGON NUT', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg_nut, 'Material': materiale_nut, 'FPD material code': codice_fpd_nut, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Ring, Wear':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='ring_gen'):
            ...
            st.session_state['output_data'] = {'Item': item, 'Description': descr, 'Identificativo': identificativo, 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ALBERO', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_BUY_1', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '24_RINGS', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Pin, Dowel':
    ...
    with col1:
        ...
        pref_df_pin = materials_df[(materials_df['Material Type'] == mtype_pin) & materials_df['Prefix'].notna()]
        ...
        if mtype_pin == 'MISCELLANEOUS':
            ...
            names_pin = materials_df[(materials_df['Material Type'] == mtype_pin) & (materials_df['Prefix'] == mprefix_pin)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='pin_gen'):
            materiale_pin = mname_pin if mtype_pin == 'MISCELLANEOUS' else f'{mtype_pin} {mprefix_pin} {mname_pin}'.strip()
            match_pin = materials_df[(materials_df['Material Type'] == mtype_pin) & (materials_df['Prefix'] == mprefix_pin) & (materials_df['Name'] == mname_pin)]
            codice_fpd_pin = match_pin['FPD Code'].values[0] if not match_pin.empty else ''
            dim_block = f'{diameter_pin} - L={length_pin}'
            descr_parts_pin = ['DOWEL PIN', dim_block, note_pin, materiale_pin, material_note_pin]
            descr_pin = '*' + ' - '.join([p for p in descr_parts_pin if p])
            st.session_state['output_data'] = {'Item': '56230…', 'Description': descr_pin, 'Identificativo': '6810-DOWEL PIN', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': materiale_pin, 'FPD material code': codice_fpd_pin, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Shaft, Pump':
    ...
    brg_size_options = {'W': ['W040', 'W050', 'W070', 'W090', 'W091', 'W091-N', 'W092', 'W092-N', 'W105', 'W105-N'], 'W-TK': ['W040-TK', 'W050-TK', 'W070-TK', 'W090-TK', 'W091-TK', 'W091-N-TK', 'W092-TK', 'W092-N-TK', 'W105-TK', 'W105-N-TK'], 'EC': ['EC0', 'EC1', 'EC2', 'EC34', 'EC5', 'EC6', 'EC7', 'EC8', 'EC85', 'EC9'], 'ECR': ['ECR0', 'ECR1', 'ECR2', 'ECR34', 'ECR5', 'ECR6', 'ECR7', 'ECR8', 'ECR85', 'ECR9'], 'ESBR': ['ESBR0', 'ESBR1', 'ESBR2', 'ESBR34', 'ESBR5', 'ESBR6', 'ESBR7', 'ESBR8', 'ESBR85', 'ESBR9'], 'EKBR': ['EKBR0', 'EKBR1', 'EKBR2', 'EKBR34', 'EKBR5', 'EKBR6', 'EKBR7', 'EKBR8', 'EKBR85', 'EKBR9'], 'ECH': ['EC0-H', 'EC2-H', 'EC5-H', 'EC6-H', 'EC7-H', 'EC8-H', 'EC85-H', 'EC9-H', 'EC10-H', 'EC11-H', 'EC12-H', 'EC15-H'], 'ESH': ['ES0-H', 'ES2-H', 'ES5-H', 'ES6-H', 'ES7-H', 'ES8-H', 'ES85-H', 'ES9-H', 'ES10-H', 'ES11-H', 'ES12-H'], 'EKH': ['EK0-H', 'EK2-H', 'EK5-H', 'EK6-H', 'EK7-H', 'EK8-H', 'EK85-H', 'EK9-H', 'EK10-H', 'EK11-H', 'EK12-H']}
    with col1:
        st.subheader('✏️ Input')
        ...
        if st.button('Generate Output', key='shaft_gen'):
            ...
            df_match = materials_df[(materials_df['Material Type'] == mtype) & (materials_df['Prefix'] == mprefix) & (materials_df['Name'] == mname)]
            codice_fpd = df_match['FPD Code'].iloc[0] if not df_match.empty else ''
            sq_tags = ['[SQ60]', '[DE3513.014]', '[CORP-ENG-0115]', '[SQ58]']
            quality_lines = ['SQ 60 - Procedura di Esecuzione del Run-Out per Alberi e Rotori di Pompe', 'DE 3513.014 - Shaft Demagnetization', 'CORP-ENG-0115 - General Surface Quality Requirements G1-1', 'SQ 58 - Controllo Visivo e Dimensionale delle Lavorazioni Meccaniche']
            astm_names = ['4140', '4140 HRC 22 max', '4140 quenched & tempered (with mechanical properties according to ASTM A434 4140 Class BC)', 'Tp. 410 Quen Temp Cd T', 'Tp. 410 - Annealed Condition A', 'Tp. 410 BHN 250-300', 'Tp. 410 DOUBLE TEMPERED HRC 22 MAX NACE', 'Tp. 410 - Double-tempered HB 237 max.', 'Tp. 410 HB 352-400', 'Tp. 410 HB 325-375', 'Tp. 410 HB 300-350', 'Tp. 410 Cond A', 'Tp. 410 Double Tempered HRC 22 max.', 'Tp. 410 Quenched & Tempered - Cond. T']
            ...
            descr = '*' + ' - '.join(descr_parts) + ' ' + tag_string
            st.session_state['output_data'] = {'Item': '40231…', 'Description': descr, 'Identificativo': '2100-SHAFT', 'Classe ricambi': '2-3', 'Categories': 'FASCIA ITE 4', 'Catalog': 'ALBERO', 'Disegno': dwg, 'Material': materiale, 'FPD material code': codice_fpd, 'Template': 'FPD_MAKE', 'ERP_L1': '20_TURNKEY_MACHINING', 'ERP_L2': '25_SHAFTS', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
    ...
    with col1:
        st.subheader('✏️ Input')
        model = st.selectbox('Pump Type', size_df['Pump Model'].dropna().unique(), key='selectbox_2')
        size = st.selectbox('Pump Size', size_df[size_df['Pump Model'] == model]['Size'].dropna().unique(), key='selectbox_3')
        ...
        filtered_names = materials_df[(materials_df['Material Type'] == mat_type) & (materials_df['Prefix'] == mat_prefix)]['Name'].dropna().drop_duplicates()
        ...
        if st.button('Generate Output'):
            ...
            descr_parts = [f'*{ident}', f'{model}-{size}', f'{length}x{width} mm', f'{weight} kg', note, material, mat_note, '[SQ53]', '[CORP-ENG-0234]']
            descr = ' '.join([d for d in descr_parts if d])
            quality = ['SQ 53 - HORIZONTAL PUMP BASEPLATES CHECKING PROCEDURE', 'CORP-ENG-0234 - Procedure for Baseplate Inspection J4-11']
            st.session_state['output_data'] = {'Item': item, 'Description': descr, 'Identificativo': ident, 'Classe ricambi': classe, 'Categories': cat, 'Catalog': catalog, 'Disegno': drawing_out, 'Material': material, 'FPD material code': fpd_code, 'Template': template, 'ERP L1': erp1, 'ERP L2': erp2, 'To Supplier': to_supplier, 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
...
if selected_part == 'Flange, Pipe':
    ...
    with col1:
        ...
        pipe_size = st.selectbox('Size', ['1/8”', '1/4”', '3/8”', '1/2”', '3/4”', '1”', '1-1/4”', '1-1/2”', '2”', '2-1/2”', '3”', '4”'], key='flange_size')
        ...
        if st.button('Generate Output', key='flange_gen'):
            descr = f'FLANGE, PIPE - TYPE: {pipe_type}, SIZE: {pipe_size}, FACE: {face_type}, CLASS: {pressure_class}, MATERIAL: {material_flange}'
            if note_flange:
            ...
            if hf_service_flange:
                ...
                quality = ''
            descr = '*' + descr
            st.session_state['output_data'] = {'Item': '50155…', 'Description': descr, 'Identificativo': '1245-FLANGE', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': '', 'Disegno': '', 'Material': 'NOT AVAILABLE', 'FPD material code': 'NA', 'Template': 'FPD_BUY_2', 'ERP_L1': '23_FLANGE', 'ERP_L2': '13_OTHER', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for campo, valore in st.session_state['output_data'].items():
...
if selected_part == 'Gasket, Flat':
    ...
    with col1:
        ...
        if st.button('Generate Output', key='gf_gen'):
            descr_gf = f'GASKET, FLAT - THICKNESS: {thickness_gf}{unit_gf.upper()}, MATERIAL: {material_gf}'
            if note_gf:
            ...
            if hf_service_gf:
                ...
                quality = ''
            descr_gf = '*' + descr_gf
            st.session_state['output_data'] = {'Item': '50158…', 'Description': descr_gf, 'Identificativo': '4590-GASKET', 'Classe ricambi': '1-2-3', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': dwg_gf, 'Material': material_gf, 'FPD material code': 'NOT AVAILABLE', 'Template': 'FPD_BUY_2', 'ERP_L1': '55_GASKETS_OR_SEAL', 'ERP_L2': '20_OTHER', 'To supplier': '', 'Quality': quality}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for campo, valore in st.session_state['output_data'].items():
...
if selected_part == 'Screw, Cap':
    ...
    with col1:
        ...
        if mtype_cap == 'MISCELLANEOUS':
            ...
            names_cap = materials_df[(materials_df['Material Type'] == mtype_cap) & (materials_df['Prefix'] == mprefix_cap)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='cap_gen'):
            materiale_cap = mname_cap if mtype_cap == 'MISCELLANEOUS' else f'{mtype_cap} {mprefix_cap} {mname_cap}'.strip()
            match_cap = materials_df[(materials_df['Material Type'] == mtype_cap) & (materials_df['Prefix'] == mprefix_cap) & (materials_df['Name'] == mname_cap)]
            codice_fpd_cap = match_cap['FPD Code'].values[0] if not match_cap.empty else ''
            descr_parts_cap = ['CAP SCREW', size_cap, length_cap, 'FULL THREADED' if full_thread_cap == 'Yes' else '', note_cap, materiale_cap, 'ZINC PLATED' if zinc_plated_cap == 'Yes' else '', material_note_cap]
            descr_cap = '*' + ' - '.join([p for p in descr_parts_cap if p])
            st.session_state['output_data'] = {'Item': '56230…', 'Description': descr_cap, 'Identificativo': '6579-SOCKET HEAD CAP SCREW', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': materiale_cap, 'FPD material code': codice_fpd_cap, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
if selected_part == 'Screw, Grub':
    ...
    with col1:
        ...
        if mtype_grub == 'MISCELLANEOUS':
            ...
            names_grub = materials_df[(materials_df['Material Type'] == mtype_grub) & (materials_df['Prefix'] == mprefix_grub)]['Name'].dropna().tolist()
        ...
        if st.button('Generate Output', key='grub_gen'):
            materiale_grub = mname_grub if mtype_grub == 'MISCELLANEOUS' else f'{mtype_grub} {mprefix_grub} {mname_grub}'.strip()
            match_grub = materials_df[(materials_df['Material Type'] == mtype_grub) & (materials_df['Prefix'] == mprefix_grub) & (materials_df['Name'] == mname_grub)]
            codice_fpd_grub = match_grub['FPD Code'].values[0] if not match_grub.empty else ''
            descr_parts_grub = ['GRUB SCREW', size_grub, length_grub, note_grub, materiale_grub, material_note_grub]
            descr_grub = '*' + ' - '.join([p for p in descr_parts_grub if p])
            st.session_state['output_data'] = {'Item': '56310…', 'Description': descr_grub, 'Identificativo': '6814-GRUB SCREW', 'Classe ricambi': '', 'Categories': 'FASCIA ITE 5', 'Catalog': 'ARTVARI', 'Disegno': '', 'Material': materiale_grub, 'FPD material code': codice_fpd_grub, 'Template': 'FPD_BUY_2', 'ERP_L1': '60_FASTENER', 'ERP_L2': '74_OTHER_FAST_COMP_EYE_NUTS_LOCK_NUTS', 'To supplier': '', 'Quality': ''}
    with col2:
        st.subheader('📤 Output')
        if 'output_data' in st.session_state:
            for k, v in st.session_state['output_data'].items():
...
col1, col2 = st.columns([1, 1], gap='small')
with col1:
    selected_category = st.selectbox('Category:', [''] + list(categories.keys()), index=0, key='selectbox_4')
with col2:
    if selected_category:
        part_list = categories[selected_category]
    else:
...
if selected_part == 'Shaft, Pump':
    ...
    with col1:
        ...
        size = st.selectbox('Pump Size', [''] + size_list, key='shaft_size')
        brg_type = st.selectbox('Bearing Type', [''] + brg_types, key='shaft_brg_type')
        brg_size = st.selectbox('Bearing Size', [''] + brg_size_options.get(brg_type, [], key='selectbox_30'), key='shaft_brg_size')
        max_diam = st.text_input('Max diameter (mm)', key='shaft_diam')
        max_len = st.text_input('Max length (mm)', key='shaft_len')
        dwg = st.text_input('Drawing number', key='shaft_dwg')
        note = st.text_area('Note', height=80, key='shaft_note')
    ...
    with col1:
        st.subheader('✏️ Input')
        model = st.selectbox('Pump Type', size_df['Pump Model'].dropna().unique(), key='selectbox_5')
        size = st.selectbox('Pump Size', size_df[size_df['Pump Model'] == model]['Size'].dropna().unique(), key='selectbox_6')
        length = st.number_input('Length (mm)', min_value=0)
        width = st.number_input('Width (mm)', min_value=0)
        weight = st.number_input('Weight (kg)', min_value=0)
        sourcing = st.selectbox('Sourcing', ['EUROPEAN', 'INDIAN', 'CHINESE'], key='selectbox_33')
        drawing = st.text_input('DWG/Doc')
        note = st.text_area('Note')
        mat_type = st.selectbox('Material Type', materials_df['Material Type'].dropna().unique(), key='base_mat_type')
        filtered_prefix = materials_df[materials_df['Material Type'] == mat_type]['Prefix'].dropna().unique()
...
if selected_part == 'Screw, Grub':
    ...
    with col3:
        render_dataload_panel(item_code_key='beye_item_code', create_btn_key='gen_dl_beye', update_btn_key='gen_upd_beye')
if selected_part in ['Casing cover casting', 'Casing casting', 'Bearing housing casting', 'Impeller casting', 'Impeller nut casting', 'Shaft casting', 'Throttling bush casting', 'Pump bowl casting', 'Bearing bracket casting', 'Discharge elbow casting', 'Bearing cover casting', 'Diffuser casting', 'Inducer casting', 'Wear plate casting', 'Shaft wear sleeve casting']:
    ...
    with col_input:
        ...
        if selected_part == 'Impeller casting':
            imp_pump_type = st.selectbox('Impeller Pump Type', ['Other', 'DMX'], key='cast_imp_pump_type')
        if selected_part == 'Bearing housing casting':
            pump_type = st.selectbox('Pump Type', ['Other', 'HPX'], key='cast_pump_type')
        ...
        names = sorted(materials_df[(materials_df['Material Type'] == material_type) & (materials_df['Prefix'] == prefix)]['Name'].dropna().unique().tolist())
    ...
    if st.session_state.cast_generated:
        with col_output:
            ...
            quality_lines = ['DE 2390.002 - Procurement and Quality Specification for Ferrous Castings', 'SQ 58 - Controllo Visivo e Dimensionale delle Lavorazioni Meccaniche', 'CORP-ENG-0115 - General Surface Quality Requirements G1-1']
            ...
            hydraulic = ['Casing cover casting', 'Casing casting', 'Impeller casting', 'Pump bowl casting', 'Diffuser casting', 'Inducer casting', 'Wear plate casting']
            if selected_part in hydraulic:
                extra = ['[DE2390.001]', '[CORP-ENG-0523]', '[CORP-ENG-0090]']
                qual_tags.extend(extra)
                quality_lines.extend(['DE 2390.001 - Procurement and Cleaning Requirements for Hydraulic Castings-API, Vertical, Submersible and Specially Pumps', 'CORP-ENG-0523 - As-Cast Surface Finish and Cleaning Requirements for Hydraulic Castings', 'CORP-ENG-0090 - Procurement and Cleaning Requirement for Hydraulic Castings - API, Vertical, Submersible, and Specialty Pumps P-5'])
            cg_materials = {('A351_', 'CG3M'), ('A351_', 'CG8M'), ('A743_', 'CG3M'), ('A743_', 'CG8M'), ('A351_', 'CG8M + HVOF TUNGS. CARBIDE 86-10-4 (WC-Co-Cr) OVERLAY'), ('A351_', 'CG3M + HVOF TUNGS. CARBIDE 86-10-4 (WC-Co-Cr) OVERLAY + PTA STELLITE 6 OVERLAY'), ('A743_', 'CG8M + PTA STELLITE 12 OVERLAY'), ('A743_', 'CG3M + PTA STELLITE 6 OVERLAY'), ('A743_', 'CG3M + DLD WC-Ni 60-40'), ('A744_', 'CG3M')}
    ...
    with col_dataload:
        ...
        elif st.button('Generate Update string', key='cast'):
            if not item_code_dl:
                st.error('❌ Please enter the item code first.')
            else:
                st.success('✅ DataLoad string successfully generated. Download the CSV below.')
